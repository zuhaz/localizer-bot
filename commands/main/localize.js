...
module.exports = {
  name: "localize",
  description: "Use this command to localize your discord server.",
  options: [
    {
      name: "language",
      description:
        "Choose the language in which you want to localize your server to.",
  ...
  async execute(interaction, client) {
    ...
    if (!localizeBackupData) {
      await new localizeBackupDB({ GuildID: interaction.guild.id }).save();
    }
    const languages = [
      {
        code: "en",
    ...
    async function formatChannels(data, interaction) {
      ...
      for (const object of data) {
        ...
        if (type === ChannelType.GuildVoice) {
          result += `> ${await translateChannels(
            name,
            code,
            object.channelid
          )}\n`;
      ...
      const embed = new EmbedBuilder()
        .setColor("#2f3136")
        .setTitle("Successfully Localized the server")
        .setAuthor({
          name: interaction.user.username,
          iconURL: interaction.user.displayAvatarURL({ dynamic: true }),
        })
        .addFields(
          { name: "Localized Into:", value: `\`${language}\``, inline: true },
          {
    ...
    async function translateChannels(
